...
sidebar <- dashboardSidebar(
  sidebarMenu(
    # Tab 1 
    menuItem(tabName = "california", "California", icon = icon("globe-americas")),
    
    # Tab 1 
    menuItem(tabName = "overview", "Overview", icon = icon("eye")),
...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(tabName = "overview",
            ...
            fluidRow(valueBoxOutput("california", width=12)),
            ...
            fluidRow(
              ...
              tabBox(
                id = "Overviews",
                width = 8,
                tabPanel("Accident Rate Ranking", DT::dataTableOutput("rankdatatable")),
...
server <- function(input, output, session) {
  ...
  # california
  output$california <- renderValueBox({
    valueBox("California", paste("Accidents in", input$year),  icon = icon("map-marker"), color = "navy")
  })
  ...
  output$accidentrank<- output$accidentrank2 <- renderInfoBox({
    infoBox("Accident Rate Ranking", paste0(data1[data1$County==input$county & data1$year == input$year,"rank"],sep = "/",length(unique(data1$County))), icon = icon("list-ol"), color = "navy")
  })
